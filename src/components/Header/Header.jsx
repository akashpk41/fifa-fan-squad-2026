      ...
      <section className="relative z-10 min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8 pt-20">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-lg sm:text-xl lg:text-2xl text-slate-600 mb-12 max-w-3xl mx-auto leading-relaxed">
            <span className="font-semibold text-slate-800">Beyond Boundaries</span>, Beyond Limits.
            <span className="block mt-2 text-base lg:text-lg font-medium">
              Create your dream team and compete with millions of players worldwide
            </span>
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            <div className="relative group">
              ...
              <button className="relative flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-yellow-400 to-orange-500 text-white font-bold text-lg rounded-2xl shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300 border-2 border-yellow-300">
            ...
            <button className="flex items-center gap-2 px-6 py-4 bg-white/60 backdrop-blur-sm border border-white/40 rounded-2xl text-slate-700 font-semibold hover:bg-white/80 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-8 max-w-4xl mx-auto">
            {[
              { icon: Users, number: "10M+", label: "Join Football Lovers in the Game", color: "text-blue-600" },
              { icon: Trophy, number: "৳100Cr+", label: "Won by Our Champions", color: "text-indigo-600" },
              { icon: Zap, number: "50K+", label: "Contests Every Day", color: "text-purple-600" },
              { icon: Star, number: "4.8★", label: "on App Stores – Users Love Us!", color: "text-yellow-600" }
            ].map((stat, index) => (
              <div key={index} className="p-4 lg:p-6 bg-white/60 backdrop-blur-sm rounded-2xl border border-white/40 hover:bg-white/80 transition-all duration-500 transform hover:scale-105 hover:shadow-lg group cursor-pointer">
                <stat.icon className={`w-6 h-6 lg:w-8 lg:h-8 ${stat.color} mx-auto mb-2 lg:mb-3 group-hover:scale-110 transition-transform duration-300`} />
                <div className="text-xl lg:text-2xl font-bold text-slate-800 mb-1">{stat.number}</div>
